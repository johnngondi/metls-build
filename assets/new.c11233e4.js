...
var CreateWorkflowStep_vue_vue_type_style_index_0_lang = "";
...
const _hoisted_3 = /* @__PURE__ */ createBaseVNode("div", { class: "left" }, [
  /* @__PURE__ */ createBaseVNode("h3", null, "Create Step")
], -1);
...
const _sfc_main$1 = defineComponent({
  __name: "CreateWorkflowStep",
  setup(__props) {
    const { y } = useWindowScroll();
    const router = useRouter();
    const route = useRoute();
    const { groups, createWorkflowStep, fetchWorkflowSteps } = useWorkflow();
    ...
    const save = async () => {
      ...
      try {
        const { data } = await createWorkflowStep(workflowId.value, __spreadValues({}, step.value));
        success("Workflow step created!");
        close();
...
const _sfc_main = defineComponent({
  __name: "new",
  setup(__props) {
    pageTitle.value = "New Workflow Step";
    return (_ctx, _cache) => {
      const _component_create_workflow_step = _sfc_main$1;
      return openBlock(), createElementBlock("div", _hoisted_1, [
        createVNode(_component_create_workflow_step)
      ]);
    };
  }
});
